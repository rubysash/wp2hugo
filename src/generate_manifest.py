...
def create_site_manifest(sqlite_db_path):
    ...
    Associates assets specifically with the posts that use them for Page Bundles.
    """
    ...
    manifest = {
        ...
        "posts": [],
        "pages": [],
        "global_assets": set() # For site-wide images if needed
    }
    ...
    for row in content_rows:
        item = dict(row)
        item_id = item["ID"]
        item["local_assets"] = [] # Assets specific to this Page Bundle
        ...
        cursor.execute(f"SELECT term_taxonomy_id FROM `{config.TABLE_MAPPING['term_relationships']}` WHERE object_id = ?", (item_id,))
        ...
        cursor.execute(f"SELECT meta_value FROM `{config.TABLE_MAPPING['postmeta']}` WHERE post_id = ? AND meta_key = '_thumbnail_id'", (item_id,))
        ...
        if thumb_id_row:
            ...
            if img_row:
                ...
                item["local_assets"].append(img_row[0])

        # Find Content Assets (Images/PDFs used inside the post)
        found_assets = re.findall(r'src=["\'](https?://[^"\']+\.(?:jpg|jpeg|png|gif|pdf|svg|webp))["\']', item["post_content"], re.IGNORECASE)
        for asset in found_assets:
            if asset not in item["local_assets"]:
                item["local_assets"].append(asset)
    ...
    total_assets = sum(len(p["local_assets"]) for p in manifest["posts"] + manifest["pages"])
    print(f"{Fore.GREEN}Manifest generated with {len(manifest['posts'])} posts and {len(manifest['pages'])} pages.{Style.RESET_ALL}")
    print(f"{Fore.WHITE}Found {total_assets} total assets mapped to Page Bundles.{Style.RESET_ALL}")
